...
if(isset($_POST['submit'])) {
    ...
    if(!$firstname) {
        ...
        $_SESSION['sign-up'] = 'Password should be 8+ characters';
    } elseif (!$avatar['name']) {
        $_SESSION['sign-up'] = 'Please upload an avatar';
    } else {
            ...
            if(mysqli_num_rows($user_check_result) > 0){
                ...
                if(in_array($extention, $allowed_files)) {
                    if($avatar['size'] < 1000000) {
                        ...
                        $_SESSION['sign-up'] = 'Avatar is too large. Should be less than 1mb';
                    }
                } else {
                    $_SESSION['sign-up'] = 'Avatar should be a jpg, jpeg or png';
                }
            }
        }
    }
    ...
    if(isset($_SESSION['sign-up'])) {
        ...
        email='$email', password='$hashedpassword', avatar='$avatar_name', is_admin=0";
        $insert_user_result = mysqli_query($connection, $insert_user_query);
        if(!mysqli_errno($connection)) {
            $_SESSION['sign-up-success'] = "You have successfully signed up";
